...
// Binary codepage search function
static void *cp_bsearch(register const void *key, register const void *base,
	register long nmemb, register long size,
...
const union cptable *cp_get_table( unsigned int codepage )
{
    ...
    if (!(res = cp_bsearch( &codepage, cptables, NB_CODEPAGES,
