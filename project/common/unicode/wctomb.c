...
static __inline int binary_search( UNICODE_WCHAR ch, int low, int high )
{
    extern const UNICODE_WCHAR unicode_compose_table[];
    while (low <= high)
    {
        int pos = (low + high) / 2;
...
static UNICODE_WCHAR compose( const UNICODE_WCHAR *str )
{
    extern const UNICODE_WCHAR unicode_compose_table[];
...
static __inline int is_valid_sbcs_mapping( const struct sbcs_table *table, int flags,
                                         UNICODE_WCHAR wch, unsigned char ch )
{
...
static __inline int wcstombs_sbcs( const struct sbcs_table *table,
                                 const UNICODE_WCHAR *src, unsigned int srclen,
...
static __inline int is_valid_dbcs_mapping( const struct dbcs_table *table, int flags,
                                         UNICODE_WCHAR wch, unsigned short ch )
{
                            ...
                            const char *defchar, int *used )
{
    ...
    const unsigned short * const uni2cp_high = table->uni2cp_high;
    UNICODE_WCHAR defchar_value = table->info.def_char;
    UNICODE_WCHAR composed;
    int len, tmp;
...
static __inline int wcstombs_dbcs( const struct dbcs_table *table,
                                 const UNICODE_WCHAR *src, unsigned int srclen,
                                 char *dst, unsigned int dstlen )
{
    const unsigned short * const uni2cp_low = table->uni2cp_low;
