...
#include "BL_alphabltsubtractive.h"
...
namespace AlphaBltSubtractive565 {
...
#ifdef _PRGM_SECT
#pragma mark ------------ RGB565 AlphaBltSubtractive Functions --------------
#endif
	...
	LONG alphaWidth, LONG alphaHeight)
{
	...
	if (bDest32Aligned && (bSrc32Aligned && bAlpha32Aligned))
	{
		...
		while (srcHeight != 0) {
			...
			*pDest32++ = AlphaBltSubtractive565::BlendPixel(dwTempSrc32 & 0x0000FFFFL,\
							dwTempDest32 & 0x0000FFFFL,\
							dwTempAlpha32 & 0x0000FFFFL) | \
						(AlphaBltSubtractive565::BlendPixel(dwTempSrc32 >> 16,\
							dwTempDest32 >> 16,\
							dwTempAlpha32 >> 16) << 16);\
			}
	...
	else if (!bDest32Aligned && (bSrc32Aligned && bAlpha32Aligned))
	{
		...
		while (srcHeight != 0) {
			...
			\
				/* Write first pixel */ \
				*pDest16++ = (WORD)AlphaBltSubtractive565::BlendPixel(dwTempSrc32 & 0x0000FFFFL,\
					*pDest16, dwTempAlpha32 & 0x0000FFFFL);\
			\
				/* Write second pixel */ \
				*pDest16++ = (WORD)AlphaBltSubtractive565::BlendPixel(dwTempSrc32 >> 16,\
	...
	else
	{
		...
		while (srcHeight != 0) {
			...
			#define BLIT_EXPR\
				*pDest16++ = (WORD)AlphaBltSubtractive565::BlendPixel(*pSrc16++, *pDest16, *pAlpha16++);
	...
	LONG alphaWidth, LONG alphaHeight, DWORD dwOpacity)
{
	...
	if (bDest32Aligned && (bSrc32Aligned && bAlpha32Aligned))
	{
		...
		while (srcHeight != 0) {
			...
			dwTempAlpha32 = *pAlpha32++;\
			dwTempDest32 = *pDest32;\
			*pDest32++ = AlphaBltSubtractive565::BlendPixelOpacity(dwTempSrc32 & 0x0000FFFFL,\
							dwTempDest32 & 0x0000FFFFL,\
							dwTempAlpha32 & 0x0000FFFFL,\
							dwOpacity) | \
						(AlphaBltSubtractive565::BlendPixelOpacity(dwTempSrc32 >> 16,\
							dwTempDest32 >> 16,\
							dwTempAlpha32 >> 16,\
							dwOpacity) << 16);\
			}
	...
	else if (!bDest32Aligned && (bSrc32Aligned && bAlpha32Aligned))
	{
		...
		while (srcHeight != 0) {
			...
			\
				/* Write first pixel */ \
				*pDest16++ = (WORD)AlphaBltSubtractive565::BlendPixelOpacity(dwTempSrc32 & 0x0000FFFFL,\
					*pDest16, dwTempAlpha32 & 0x0000FFFFL, dwOpacity);\
			\
				/* Write second pixel */ \
				*pDest16++ = (WORD)AlphaBltSubtractive565::BlendPixelOpacity(dwTempSrc32 >> 16,\
	...
	else
	{
		...
		while (srcHeight != 0) {
			...
			#define BLIT_EXPR\
				*pDest16++ = (WORD)AlphaBltSubtractive565::BlendPixelOpacity(*pSrc16++, *pDest16, *pAlpha16++, dwOpacity);
