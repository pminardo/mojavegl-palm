...
#ifdef _PRGM_SECT
#pragma mark ------------ RGB565 StretchAlphaBlt Functions --------------
#endif
		...
		(dwFlags & MGLSTRETCHALPHABLT_MIRRORUPDOWN))
	{
		...
		while (pDest16 < pDestEnd16)
		{
			...
			{\
				...
				*destp++ = StretchAlphaBlt565::BlendPixel(*(srcp - nSrcOffset), *destp, *(alphap - nSrcOffset));\
	...
	RECT *pClipRect, DWORD dwFlags, DWORD dwOpacity)
{
		...
		(dwFlags & MGLSTRETCHALPHABLT_MIRRORUPDOWN))
	{
		...
		while (pDest16 < pDestEnd16)
		{
			...
			{\
				nSrcOffset = (currXFixed >> 16);\
				*destp++ = StretchAlphaBlt565::BlendPixelOpacity(*(srcp - nSrcOffset), *destp, *(alphap - nSrcOffset), dwOpacity);\
				currXFixed += xScaleFactorFixed;\
			}
