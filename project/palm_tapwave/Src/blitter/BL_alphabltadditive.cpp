...
namespace AlphaBltAdditive565 {
	...
	DWORD BlendPixel(DWORD pixel, DWORD backpixel, DWORD alphapixel)
	{
		...
		{
			if (alpha < MAXALPHA)
				return pxutil565::AlphaBlendAdditive(pixel, backpixel, alpha);
...
#ifdef _PRGM_SECT
#pragma mark ------------ RGB565 AlphaBltAdditive Functions --------------
#endif
	...
	LONG alphaWidth, LONG alphaHeight)
{
	...
	if (bDest32Aligned && (bSrc32Aligned && bAlpha32Aligned))
	{
		...
		while (srcHeight != 0) {
			...
			*pDest32++ = AlphaBltAdditive565::BlendPixel(dwTempSrc32 & 0x0000FFFFL,\
							dwTempDest32 & 0x0000FFFFL,\
							dwTempAlpha32 & 0x0000FFFFL) | \
						(AlphaBltAdditive565::BlendPixel(dwTempSrc32 >> 16,\
							dwTempDest32 >> 16,\
							dwTempAlpha32 >> 16) << 16);\
			}
	...
	else if (!bDest32Aligned && (bSrc32Aligned && bAlpha32Aligned))
	{
		...
		while (srcHeight != 0) {
			...
			\
				/* Write first pixel */ \
				*pDest16++ = (WORD)AlphaBltAdditive565::BlendPixel(dwTempSrc32 & 0x0000FFFFL,\
					*pDest16, dwTempAlpha32 & 0x0000FFFFL);\
			\
				/* Write second pixel */ \
				*pDest16++ = (WORD)AlphaBltAdditive565::BlendPixel(dwTempSrc32 >> 16,\
	...
	else
	{
		...
		while (srcHeight != 0) {
			...
			#define BLIT_EXPR\
				*pDest16++ = (WORD)AlphaBltAdditive565::BlendPixel(*pSrc16++, *pDest16, *pAlpha16++);
	...
	LONG alphaWidth, LONG alphaHeight, DWORD dwOpacity)
{
	...
	if (bDest32Aligned && (bSrc32Aligned && bAlpha32Aligned))
	{
		...
		while (srcHeight != 0) {
			...
			dwTempAlpha32 = *pAlpha32++;\
			dwTempDest32 = *pDest32;\
			*pDest32++ = AlphaBltAdditive565::BlendPixelOpacity(dwTempSrc32 & 0x0000FFFFL,\
							dwTempDest32 & 0x0000FFFFL,\
							dwTempAlpha32 & 0x0000FFFFL,\
							dwOpacity) | \
						(AlphaBltAdditive565::BlendPixelOpacity(dwTempSrc32 >> 16,\
							dwTempDest32 >> 16,\
							dwTempAlpha32 >> 16,\
							dwOpacity) << 16);\
			}
	...
	else if (!bDest32Aligned && (bSrc32Aligned && bAlpha32Aligned))
	{
		...
		while (srcHeight != 0) {
			...
			\
				/* Write first pixel */ \
				*pDest16++ = (WORD)AlphaBltAdditive565::BlendPixelOpacity(dwTempSrc32 & 0x0000FFFFL,\
					*pDest16, dwTempAlpha32 & 0x0000FFFFL, dwOpacity);\
			\
				/* Write second pixel */ \
				*pDest16++ = (WORD)AlphaBltAdditive565::BlendPixelOpacity(dwTempSrc32 >> 16,\
	...
	else
	{
		...
		while (srcHeight != 0) {
			...
			#define BLIT_EXPR\
				*pDest16++ = (WORD)AlphaBltAdditive565::BlendPixelOpacity(*pSrc16++, *pDest16, *pAlpha16++, dwOpacity);
