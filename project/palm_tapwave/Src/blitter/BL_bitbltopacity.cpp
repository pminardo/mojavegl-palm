...
* All rights reserved.
*
* NOTE: This code is identical to BL_bitblt.cpp except
* for the addition of the dwOpacity blending value.
*
...
#ifdef _PRGM_SECT
#pragma mark ------------ RGB565 BitBltOpacity Functions --------------
#endif
	...
	if (bDest32Aligned && bSrc32Aligned)
	{
		...
		if (dwOpacity == OPACITY_50)
		{
			while (srcHeight != 0) {
				...
				#define BLIT_EXPR\
					*pDest32++ = pxutil565::AlphaBlendFast32(*pSrc32++, *pDest32);
		...
		else
		{
			while (srcHeight != 0) {
				...
				#define BLIT_EXPR\
					*pDest32++ = pxutil565::AlphaBlend32(*pSrc32++, *pDest32, dwOpacity);
	...
	else if (bSrc32Aligned)
	{
		...
		if (dwOpacity == OPACITY_50)
		{
			while (srcHeight != 0) {
				...
				\
					...
					*pDest16++ = (WORD)pxutil565::AlphaBlendFast(dwTempSrc32 & 0x0000FFFFL, *pDest16);\
					*pDest16++ = (WORD)pxutil565::AlphaBlendFast(dwTempSrc32 >> 16, *pDest16);\
				}
		...
		else
		{
			while (srcHeight != 0) {
				...
				\
					/* Write 2 pixels on destination image */ \
					*pDest16++ = (WORD)pxutil565::AlphaBlend(dwTempSrc32 & 0x0000FFFFL, *pDest16, dwOpacity);\
					*pDest16++ = (WORD)pxutil565::AlphaBlend(dwTempSrc32 >> 16, *pDest16, dwOpacity);\
				}
	...
	else
	{
		...
		if (dwOpacity == OPACITY_50)
		{
			while (srcHeight != 0) {
				...
				#define BLIT_EXPR\
					*pDest16++ = (WORD)pxutil565::AlphaBlendFast(*pSrc16++, *pDest16);\
		...
		else
		{
			while (srcHeight != 0) {
				...
				#define BLIT_EXPR\
					*pDest16++ = (WORD)pxutil565::AlphaBlend(*pSrc16++, *pDest16, dwOpacity);\
	...
	LONG srcWidth, LONG srcHeight, DWORD dwNativeColorKey, DWORD dwOpacity)
{
	...
	if (bDest32Aligned && bSrc32Aligned)
	{
		...
		DWORD dwDestOffset;
		DWORD dwSrcOffset;
		DWORD dwTempSrc32, dwTempDest32, dwResult32;
		...
		if (dwOpacity == OPACITY_50)
		{
			while (srcHeight != 0) {
				...
				\
					...
					{\
						dwResult32 = dwTempDest32 = *pDest32;\
					\
						...
						{\
							...
							dwResult32 |= pxutil565::AlphaBlendFast(dwTempSrc32 & 0x0000FFFFL,\
								dwTempDest32 & 0x0000FFFFL);\
						}\
						if ((dwTempSrc32 >> 16) != dwNativeColorKey)\
						{\
							dwResult32 &= 0x0000FFFFL;	/*clear hi-word to 0*/ \
							dwResult32 |= pxutil565::AlphaBlendFast(dwTempSrc32 >> 16,\
								dwTempDest32 >> 16) << 16;\
						}\
						\
						*pDest32 = dwResult32;\
			...
			while (srcHeight != 0) {
				...
				\
					...
					\
						...
						{\
							dwResult32 &= 0xFFFF0000L;	/*clear low-word to 0*/ \
							dwResult32 |= pxutil565::AlphaBlend(dwTempSrc32 & 0x0000FFFFL,\
								dwTempDest32 & 0x0000FFFFL, dwOpacity);\
						}\
						if ((dwTempSrc32 >> 16) != dwNativeColorKey)\
						{\
							dwResult32 &= 0x0000FFFFL;	/*clear hi-word to 0*/ \
							dwResult32 |= pxutil565::AlphaBlend(dwTempSrc32 >> 16,\
								dwTempDest32 >> 16, dwOpacity) << 16;\
						}\
						\
						*pDest32 = dwResult32;\
					}\
			...
			while (srcHeight != 0) {
				...
				{\
					...
					if ((dwTempSrc16 = *pSrc16) != dwNativeColorKey)\
						*pDest16 = (WORD)pxutil565::AlphaBlendFast(dwTempSrc16, *pDest16);\
					\
					++pSrc16;\
					++pDest16;\
				}
		...
		else
		{
			while (srcHeight != 0) {
				...
				{\
					/* Copy pixel if it doesn't match the colorkey */ \
					if ((dwTempSrc16 = *pSrc16) != dwNativeColorKey)\
						*pDest16 = (WORD)pxutil565::AlphaBlend(dwTempSrc16, *pDest16, dwOpacity);\
					\
					++pSrc16;\
					++pDest16;\
				}
			...
			while (srcHeight != 0) {
				...
				\
					...
					\
						...
						{\
							dwResult32 &= 0xFFFF0000L;	/* clear low-word to 0 */ \
							dwResult32 |= pxutil565::AlphaBlendFast(dwNativeColorFill, *pDest32 & 0x0000FFFFL);\
						}\
						if ((dwTempSrc32 >> 16) != dwNativeColorKey)\
						{\
							dwResult32 &= 0x0000FFFFL;	/* clear hi-word to 0 */ \
							dwResult32 |= pxutil565::AlphaBlendFast(dwNativeColorFill, *pDest32 >> 16) << 16;\
						}\
						\
						*pDest32 = dwResult32;\
					}\
			...
			while (srcHeight != 0) {
				...
				\
					...
					\
						...
						{\
							dwResult32 &= 0xFFFF0000L;	/* clear low-word to 0 */ \
							dwResult32 |= pxutil565::AlphaBlend(dwNativeColorFill, *pDest32 & 0x0000FFFFL, dwOpacity);\
						}\
						if ((dwTempSrc32 >> 16) != dwNativeColorKey)\
						{\
							dwResult32 &= 0x0000FFFFL;	/* clear hi-word to 0 */ \
							dwResult32 |= pxutil565::AlphaBlend(dwNativeColorFill, *pDest32 >> 16, dwOpacity) << 16;\
						}\
						\
						*pDest32 = dwResult32;\
					}\
			...
			while (srcHeight != 0) {
				...
				\
					...
					{\
						/* Write first pixel on destination image */ \
						if ((dwTempSrc32 & 0x0000FFFFL) != dwNativeColorKey)\
							*pDest16 = (WORD)pxutil565::AlphaBlendFast(dwNativeColorFill, *pDest16);\
					\
						/* Skip to next pixel */ \
						++pDest16;\
					\
			...
			while (srcHeight != 0) {
				...
				\
					...
					{\
						/* Write first pixel on destination image */ \
						if ((dwTempSrc32 & 0x0000FFFFL) != dwNativeColorKey)\
							*pDest16 = (WORD)pxutil565::AlphaBlend(dwNativeColorFill, *pDest16, dwOpacity);\
					\
						/* Skip to next pixel */ \
						++pDest16;\
					\
