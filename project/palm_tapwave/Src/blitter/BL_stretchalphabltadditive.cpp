...
#include "BL_stretchalphabltadditive.h"
...
namespace StretchAlphaBltAdditive565 {
...
#ifdef _PRGM_SECT
#pragma mark ------------ RGB565 StretchAlphaBltAdditive Functions --------------
#endif
	...
	RECT *pClipRect, DWORD dwFlags)
{
	...
	// -------------------------------------------
	// Do the StretchAlphaBltAdditive()!
	// -------------------------------------------
		...
		(dwFlags & MGLSTRETCHALPHABLT_MIRRORUPDOWN))
	{
		...
		while (pDest16 < pDestEnd16)
		{
			...
			{\
				...
				*destp++ = StretchAlphaBltAdditive565::BlendPixel(*(srcp - nSrcOffset), *destp, *(alphap - nSrcOffset));\
	...
	RECT *pClipRect, DWORD dwFlags, DWORD dwOpacity)
{
		...
		(dwFlags & MGLSTRETCHALPHABLT_MIRRORUPDOWN))
	{
		...
		while (pDest16 < pDestEnd16)
		{
			...
			{\
				nSrcOffset = (currXFixed >> 16);\
				*destp++ = StretchAlphaBltAdditive565::BlendPixelOpacity(*(srcp - nSrcOffset), *destp, *(alphap - nSrcOffset), dwOpacity);\
				currXFixed += xScaleFactorFixed;\
			}
